...
class Solution {
    ...
    public Node correctBST(Node root) {
        ...
        for(int i=0;i<n;i++)
        {
            Node current=ans.get(i);
            if(i==0)
            {
                Node next=ans.get(i+1);
            ...
            else
            {
                Node next=ans.get(i+1);
                Node prev=ans.get(i-1);
                if(!(next.data<current.data && prev.data>current.data) && (next.data<current.data || prev.data>current.data))
                {
                    if(node1==null)
                    {
                        node1=current;
